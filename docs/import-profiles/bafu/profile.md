...
Process curation must consume finalized flow and support records. It must refresh global references before validation instead of failing late on stale refs. Required process fields such as reference year and annual supply/production must be filled from traceable source evidence or an explicitly documented package-level fallback rule. For `annualSupplyOrProductionVolume`, real source evidence — a single object or a language array — is preserved byte-semantically and in original element order, and no quantity is ever invented from a reference flow or a default unit. Genuinely missing source evidence is normalized by Foundry's deterministic cleanup to the supported empty array `[]` with a bound row-level evidence gap rather than `common:other` deferral or a fabricated placeholder; the historical `9999 missing-data-sentinel/year` value remains only as a read-only recognition marker for rows written by earlier rounds.
