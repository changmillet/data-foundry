...
- `common:other` 只能保存不能安全推断的 provenance/trace，不得替代 schema 必填字段或正式名称、functional unit、classification、reference flow 等业务字段。只有 action item 的 `allowed_resolution_modes` 明确包含 `deferred_to_common_other` 时，才允许延期；functional unit / classification / reference flow 这类正式字段必须补 evidence-backed 真值或保持阻塞。`annualSupplyOrProductionVolume` 是 schema 必填项，不得转入 `common:other`；真实值（单对象或语言数组）按字节语义与原始顺序保留，不得因数值与历史标记相同而改写，也不得从 reference flow 或默认 unit 造量；若原始数据确实缺少年供应量，Foundry deterministic cleanup 将其规范为受支持的未知表示 `[]`，并在 `annual_supply_evidence_gaps` 中写入绑定了行身份、字段与前后 payload SHA 的证据缺口，绝不再写入伪造数量。历史值 `9999 missing-data-sentinel/year` 仅作为只读历史识别标记，用于规范化早期写入的行。允许 `resolution.mode=deferred_to_common_other` 的 action item 必须写入 `common:other.tiangongfoundry:unresolvedTrace`，且包含 `status`、`action_item_code`、`blocked_path`、`reason`、结构化 `evidence` 和 `next_action`；`evidence` 必须包含 source 以及 quote / trace / path / citation 等可追踪指针。
