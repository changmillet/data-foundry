...
| `tiangong-foundry task resume --workspace <path> --task <id> --actor <id> [--semantic-input <file> | --interaction-input <file> | --authorization-input <file> | --reference-input <file>] --json` | Continue registered preparation, capture one bound interaction/semantic/reference/approval input, or execute and verify a sealed owner scope. Input kinds are mutually exclusive. Consumed/ambiguous mutation state is readback-only and never replayed. |
...
`tiangong-foundry.task-start.v1` contains exact request and actor ids, one of the three lanes, profile, ordered target entity types, selected source paths, optional account intent, an optional selected JSON seed and at most one current local preparation (`dataset-curation-cleanup`). The source-evidence lane requires its seed to be one of the selected sources. Task start performs no authentication and stores no identity receipt.

An optional inline `brief` keeps the task's original user wording (or `null` when the caller retains that evidence elsewhere), a concise goal, optional intended use and scope, expected deliverables, user constraints and separately identified AI assumptions. The field is omitted for older task-start v1 specs; parsing does not add a default, so their serialized shape and revision fingerprints stay unchanged. Changing a registered brief through task start creates a retained request revision under the ordinary predecessor and no-replay rules. The current brief is projected as a task artifact; it is task-local context, not source-data proof or write authority.
...
## Existing owner-draft metadata repair

The `existing-owner-draft-repair` lane selects exactly three distinct source files: a native execution contract, complete before rows and candidate rows. It currently accepts Process rows, stable identity/version and existing ownership/source-reference description text only. Scientific values, references, language structure and annual-volume content cannot change. Registration and status are offline. Resume uses the qualified installed CLI for contract dry-run and exact current-owner/state-0 before readback; producer-indexed raw owner reports and per-root JSONL proofs must agree with the complete selected content. Summaries alone do not prove readback. A no-change scope completes only after validation and exact readback of both selections, without an execution request or mutation attempt.

For this metadata-only lane, reference integrity is distinct from reference freshness. Expectations are derived from the complete selected before/candidate payloads and every unchanged reference occurrence. An exact historical reference may remain only when the CLI proves that requested version exists, its sole finding is a newer version, all blocker/count/check records agree, and every root has exact current owner/state/content readback. The raw blocked CLI report and freshness findings remain unchanged; the preparation and closeout retain a separate proof and `publication_ready: false`. A missing exact version, changed reference, mismatched input path, unexpected failure or incomplete check set blocks repair. Ordinary authoring and canonical-reference quality gates keep their existing latest-version requirements.

A changed scope requires `input_kind: repair_rows` authorization, an explicit `repair_preparation` reference and the matching native contract. The existing descriptor's `finalization_sha256` binds that preparation digest for this explicit kind; no generic finalization report is manufactured. Only the locked generic profile is currently admitted for repair writes. The grant/capsule/owner-request stores are shared with ordinary execution. Dispatch rechecks the immutable preparation, account, candidate and native contract, then uses the same CLI-owned attempt ledger and guarded save path. Unknown annual volume retains its failed authoring-evidence layer and false publication readiness.

Repair closeout requires the matching consumed native execution receipt plus independent exact owner/state/payload and reference-closure readback. It binds the same preparation report, never claims generic authoring/QA/location checks ran, and never accepts trace-normalized equality. A missing receipt or wrong readback remains unresolved; resume performs readback only and cannot repeat the mutation. Status rehashes retained proof and may complete a verified repair without a generic row-authoring finalization. An explicitly linked successor may proceed only when the predecessor has a producer-verified terminal CLI failure proving zero executed actions and zero consumed CLI attempts. It preserves the same request/account/identities, uses new immutable input paths and a genuinely changed desired payload, and requires a fresh preflight and grant. A fresh before image is permitted; old evidence remains immutable. Unknown or dispatched predecessors remain blocked.
...
`task resume --interaction-input <file>` selects a bounded `tiangong-foundry.interaction-input.v1` descriptor with exact `task_id`, `actor_id`, `expected_state_sha256` and 1–16 `events`. The first submission uses `expected_state_sha256: null`; later submissions name the current indexed `interaction-state.json` artifact SHA-256. A question event records a stable id, task-wide or dataset-type scope, what is missing, result impact, recommendation, the actual short question, up to four choices, registered evidence hashes and an optional superseded question id. A `support` scope applies to its concrete contact/source/unitgroup/flowproperty sets. An answer records the question id, a unique decision id, `supersedes_decision_id` (`null` first, then the exact previous decision id), the raw reply, its interpreted decision or `null`, `decided` or `investigate`, and registered evidence hashes. An AI assumption records its own id, scope, statement, impact, evidence and optional superseded assumption id in the same scope. Required nullable/list fields are explicit, as defined in `specs/schemas/foundry-interaction-input.schema.json`.

The descriptor and exact raw reply are captured as immutable task artifacts under the existing lock and hash-chained index, with a bounded state-file size. Duplicate identical submissions are idempotent; a stale state digest, altered input, unknown question, invented evidence hash, duplicate id, or out-of-task dataset scope is refused. Corrected answers append a new decision with an explicit predecessor while retaining the original reply; `investigate` does not become consent. New question text is supplied by the entry skill from current evidence, and Foundry projects it immediately in a human-readable `next_action` with decision-critical missing facts, impact, recommendation and response choices. The full evidence remains available as indexed artifacts. A pending question or investigation prevents write approval and completion while independent local assessment continues. Once an owner execution request is registered, new interaction events require a separately reviewed task revision; the request and consumed attempt remain bound to their original evidence.

The interaction state is separate from `semantic-input`, scientific source facts and write authorization. Each assessed set may register `interaction-context.json` containing only applicable questions, user choices and AI assumptions; task-wide items apply to every set. A set-level digest controls reuse, so changing a Process choice does not invalidate an unrelated Flow assessment. Semantic submissions after an interaction exists name its current state SHA-256 and exactly the applicable `decision_ids` for each selected work item. The runtime verifies these bindings before applying a patch or decision, then records them with the existing owner result; a user choice still cannot replace evidence required by the CLI and scientific gates. Tasks with a brief or interaction state project a concise derived `decision_recap` source at every stage, including partial local results; it carries a raw-answer hash and points back to the indexed full state instead of repeating long private text in the default result. `completion_proven` becomes true only on verified completion. The entry skill turns that source into short prose with actual completion limits. Legacy tasks without new context retain their prior result shape, and the recap adds no approval or completion gate.
