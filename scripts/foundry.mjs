  ...
  if (options.help) {
    return {
      ...
      usage: [
        ...
        "npm run legacy:dataset:curation-queue:build -- --processes ./rows/processes.jsonl --flows ./rows/flows.jsonl --support ./rows/sources.jsonl --out-dir ./curation-queue",
      ],
      foundry_wrapper: {
        exit_code: 0,
  ...
  if (options.help) {
    return {
      ...
      usage: [
        ...
        "npm run legacy:dataset:patch:apply -- --input ./rows/processes.jsonl --patch ./curation/patches.json --out ./rows/processes.patched.jsonl --out-dir ./patch-apply --authoring-package-dir ./curation-gate/ai-authoring-packages --require-action-item-closure",
      ],
      foundry_wrapper: {
        exit_code: 0,
        owner: "tiangong-lca-cli",
...
function knownCommitReportForDatasetType(datasetType, handoffDir) {
  if (datasetType === "flow") {
    return path.join(
      handoffDir,
      "commit",
...
function knownPostWriteVerifyReport(handoffDir) {
  return path.join(
    handoffDir,
    "post-write-verify",
    "outputs",
...
function supportSaveDraftArgs({ rowsFile, outDir, commit }) {
  return [
    resolveTiangongLcaCliBin(),
    "dataset",
    "save-draft",
...
function supportVerifyArgs({ rowsFile, outDir, targetUserId, stateCode }) {
  const args = [
    resolveTiangongLcaCliBin(),
    "dataset",
    "verify-remote",
...
function knownSupportSaveDraftReport(outDir) {
  return path.join(outDir, "outputs", "dataset-save-draft", "summary.json");
}

function knownSupportVerifyReport(outDir) {
  return path.join(outDir, "outputs", "remote-verification-report.json");
}

function stripFoundryInvocationArgs(args) {
  const values = [...ensureArray(args)].map((item) => String(item));
  while (values.length > 0) {
    ...
    if (
      first === "scripts/foundry.mjs" ||
      first === "./scripts/foundry.mjs" ||
      first === path.join(repoRoot, "scripts", "foundry.mjs")
    ) {
      values.shift();
      continue;
    }
...
function readFoundryArgsFile(value) {
  const filePath = resolveRepoPath(value);
  if (!filePath) return null;
  if (!fileExists(filePath)) {
    throw new Error(`Foundry args file is not readable: ${value}`);
  }
  ...
  if (!args) {
    throw new Error(
      `Foundry args file must be a JSON array or object with args array: ${value}`,
    );
  }
  const normalized = stripFoundryInvocationArgs(args);
  if (normalized[0] !== "dataset-post-authoring-finalize") {
    throw new Error(
      `Foundry finalize args must start with dataset-post-authoring-finalize: ${value}`,
    );
  }
  return {
    path: filePath,
...
function argsIncludeOption(args, optionName) {
  const camelName = optionName.slice(2).replace(/-([a-z])/gu, (_, letter) =>
    letter.toUpperCase(),
  );
  const parsed = parseArgs(args.slice(1));
  return Object.hasOwn(parsed, camelName);
}

function finalizeArgsWithDefaults(args, { outDir, targetUserId, stateCode }) {
  const next = [...args];
  if (!argsIncludeOption(next, "--out-dir")) {
    next.push("--out-dir", outDir);
  }
  if (targetUserId && !argsIncludeOption(next, "--target-user-id")) {
    next.push("--target-user-id", targetUserId);
  }
  if (stateCode && !argsIncludeOption(next, "--state-code")) {
    next.push("--state-code", stateCode);
  }
  if (!argsIncludeOption(next, "--json")) {
    next.push("--json");
  }
  return next;
}

function finalizeReportFileFromArgs(args) {
  const parsed = parseArgs(args.slice(1));
  const outDir = resolveRepoPath(parsed.outDir);
  return outDir
    ? path.join(outDir, "dataset-post-authoring-finalize-report.json")
    : null;
}

function rowsFileFromFinalizeArgs(args) {
  const parsed = parseArgs(args.slice(1));
  return resolveRepoPath(parsed.rowsFile || parsed.input || parsed.rows);
}

function foundryStageCommandArgs(args) {
  return [process.execPath, path.join(repoRoot, "scripts", "foundry.mjs"), ...args];
}

function runJsonCommandStage(stage, args, reportFile = null) {
  ...
  return {
    ...
    status:
      report?.status ??
      (exitCode === 0 && !parse_error ? "completed" : "failed"),
    exit_code: exitCode,
    command: args[0],
    args: args.slice(1),
    command_line: args.map(shellQuote).join(" "),
    stderr: result.stderr || "",
    report,
    report_file: reportFile && fileExists(reportFile) ? reportFile : null,
    parse_error,
  };
}

function plannedCommandStage(stage, args, reportFile = null) {
  return {
    stage,
    status: "planned",
    exit_code: 0,
...
function stageFailureBlocker(stage) {
  if (!stage) return null;
  const status = asText(stage.status ?? stage.report?.status);
  const failedCount = Number(
    stage.report?.counts?.failed ?? stage.report?.counts?.failure_count ?? 0,
  );
  const semanticFailure =
    /blocked|failed|failure|with_failures/u.test(status) ||
    (Number.isFinite(failedCount) && failedCount > 0);
  if (stage.exit_code === 0 && !semanticFailure) return null;
  return {
    code: `${stage.stage}_failed`,
    stage: stage.stage,
    message: `${stage.stage} failed during ordered import commit run.`,
    exit_code: stage.exit_code,
    status: status || null,
    failed_count: Number.isFinite(failedCount) ? failedCount : null,
    report_file: repoRelativeMaybe(stage.report_file),
    parse_error: stage.parse_error ?? null,
    stderr: stage.stderr ? String(stage.stderr).slice(0, 1000) : "",
  };
}

function closeoutFailureBlockers(closeout, stage) {
  if (closeout?.status === "completed") return [];
  return [
    {
      code: `${stage}_closeout_not_completed`,
      stage,
      message: `${stage} post-write closeout did not complete.`,
      closeout_report: closeout?.files?.report ?? null,
      closeout_status: closeout?.status ?? null,
      closeout_blockers: closeout?.blockers ?? [],
    },
  ];
}

function importCommitHandoffScope({
  datasetType,
  finalizeArtifact,
  targetUserId,
  stateCode,
  ...
  if (!finalizeArtifact) return null;
  const handoff = runDatasetCommitHandoffPlan({
    finalizeReport: finalizeArtifact.path,
    targetUserId,
    stateCode,
    outDir,
  });
  if (handoff.status !== "ready_for_explicit_commit") {
    blockers.push(
      ...ensureArray(handoff.blockers).map((blocker) => ({
        ...blocker,
        stage: `${datasetType}_handoff_plan`,
      })),
    );
  }
  return handoff;
...
function reusableFinalizeArtifactFromArgs({ datasetType, finalizeArgs }) {
  const reportPath = finalizeReportFileFromArgs(finalizeArgs);
  const artifact =
    reportPath && fileExists(reportPath)
      ? { path: reportPath, value: readJson(reportPath) }
      : null;
  if (!artifact) return null;
  const expectedRowsFile = rowsFileFromFinalizeArgs(finalizeArgs);
  const actualRowsFile = resolveRepoPath(
    artifact.value?.rows_file ||
      artifact.value?.pre_cleanup_rows_file ||
      artifact.value?.input_path,
  );
  if (
    expectedRowsFile &&
    actualRowsFile &&
    !sameResolvedPath(expectedRowsFile, actualRowsFile)
  ) {
    return {
      path: artifact.path,
      ...
      reuse_blockers: [
        {
          code: `${datasetType}_finalize_reuse_rows_mismatch`,
          stage: `${datasetType}_post_authoring_finalize`,
          message:
            "Existing finalize report rows_file does not match the delayed finalize args rows-file; rerun finalize instead of reusing stale evidence.",
          expected_rows: repoRelativeMaybe(expectedRowsFile),
          actual_rows: repoRelativeMaybe(actualRowsFile),
          report: repoRelativePath(artifact.path),
        },
      ],
    };
  }
  if (artifact.value?.dataset_type && artifact.value.dataset_type !== datasetType) {
    return {
      path: artifact.path,
      value: artifact.value,
      reuse_blockers: [
        {
          code: `${datasetType}_finalize_reuse_dataset_type_mismatch`,
          stage: `${datasetType}_post_authoring_finalize`,
          message:
            "Existing finalize report dataset_type does not match the requested delayed finalize scope.",
          expected_dataset_type: datasetType,
          actual_dataset_type: artifact.value.dataset_type,
          report: repoRelativePath(artifact.path),
        },
      ],
    };
...
function runDatasetImportCommitRun(options) {
  if (options.help) {
    return {
      schema_version: 1,
      status: "help",
      command: "dataset-import-commit-run",
      usage: [
        "node scripts/foundry.mjs dataset-import-commit-run --support-rows <support.jsonl> --flow-finalize-report <flow-finalize.json> --process-finalize-report <process-finalize.json> --target-user-id <uuid> --state-code 0 --out-dir <dir>",
        "node scripts/foundry.mjs dataset-import-commit-run --support-rows <support.jsonl> --flow-finalize-args-file <flow-finalize-args.json> --process-finalize-args-file <process-finalize-args.json> --target-user-id <uuid> --state-code 0 --out-dir <dir>",
        "node scripts/foundry.mjs dataset-import-commit-run --support-rows <support.jsonl> --flow-finalize-args-file <flow-finalize-args.json> --process-finalize-args-file <process-finalize-args.json> --reuse-ready-finalize-reports --target-user-id <uuid> --state-code 0 --out-dir <dir>",
        "node scripts/foundry.mjs dataset-import-commit-run --support-rows <support.jsonl> --flow-finalize-report <flow-finalize.json> --process-finalize-report <process-finalize.json> --target-user-id <uuid> --state-code 0 --out-dir <dir> --commit --confirm-remote-write",
      ],
      purpose:
        "Run the ordered Foundry remote-write handoff for a ready import scope: support rows first, then flows, then processes, with post-write verify and closeout evidence.",
      remote_write_mode: "destructive-write-only-with---commit",
    };
  }

  const outDir = resolveRepoPath(
    options.outDir || ".foundry/workspaces/dataset-import-commit-run",
  );
  const supportRowsFile = resolveRepoPath(
    options.supportRows || options.supportRowsFile || options.support,
  );
  const flowFinalizeArtifact = readJsonArtifactOption(
    options.flowFinalizeReport || options.flowReport,
  );
  const processFinalizeArtifact = readJsonArtifactOption(
    options.processFinalizeReport || options.processReport,
  );
  const flowFinalizeArgsSpec = readFoundryArgsFile(
    options.flowFinalizeArgsFile || options.flowFinalizeArgs,
  );
  const processFinalizeArgsSpec = readFoundryArgsFile(
    options.processFinalizeArgsFile || options.processFinalizeArgs,
  );
  const targetUserId = asText(
    options.targetUserId ||
      flowFinalizeArtifact?.value?.target_user_id ||
      processFinalizeArtifact?.value?.target_user_id,
  );
  const stateCode = asText(options.stateCode ?? options.expectedStateCode);
  const commitRequested = booleanOption(options.commit || options.execute);
  const confirmRemoteWrite = booleanOption(
    options.confirmRemoteWrite || options.confirmWrite,
  );
  const reuseReadyFinalizeReports = booleanOption(
    options.reuseReadyFinalizeReports || options.reuseFinalizeReports,
  );
  const blockers = [];
  const stages = [];
  const closeoutReportPaths = [];
  let importCompletionReport = null;

  if (
    !supportRowsFile &&
    !flowFinalizeArtifact &&
    !flowFinalizeArgsSpec &&
    !processFinalizeArtifact &&
    !processFinalizeArgsSpec
  ) {
    blockers.push({
      code: "import_commit_scope_required",
      message:
        "Provide at least one of --support-rows, --flow-finalize-report/--flow-finalize-args-file, or --process-finalize-report/--process-finalize-args-file.",
    });
  }
  if (flowFinalizeArtifact && flowFinalizeArgsSpec) {
    blockers.push({
      code: "flow_finalize_scope_ambiguous",
      message:
        "Provide either --flow-finalize-report or --flow-finalize-args-file, not both.",
    });
  }
  if (processFinalizeArtifact && processFinalizeArgsSpec) {
    blockers.push({
      code: "process_finalize_scope_ambiguous",
      message:
        "Provide either --process-finalize-report or --process-finalize-args-file, not both.",
    });
  }
  if (supportRowsFile && !fileExists(supportRowsFile)) {
    blockers.push({
      code: "support_rows_missing",
      message: "--support-rows must point to a readable support JSON/JSONL file.",
      support_rows: repoRelativeMaybe(supportRowsFile),
    });
  }
  if (!targetUserId) {
    blockers.push({
      code: "target_user_id_required",
      message:
        "Ordered import commit requires --target-user-id so post-write verification can prove ownership.",
    });
  }
  if (!stateCode) {
    blockers.push({
      ...
      message:
        "Ordered import commit requires --state-code so post-write verification can prove the committed scope.",
    });
  }
  if (commitRequested && !confirmRemoteWrite) {
    blockers.push({
      code: "confirm_remote_write_required",
      message:
        "Remote writes require --commit --confirm-remote-write. Omit --commit to generate a read-only execution plan.",
    });
  }

  const supportDryRunDir = path.join(outDir, "support", "dry-run");
  const supportCommitDir = path.join(outDir, "support", "commit");
  const supportVerifyDir = path.join(outDir, "support", "post-write-verify");
  const supportCommands = supportRowsFile
    ? {
        dry_run: supportSaveDraftArgs({
          rowsFile: supportRowsFile,
          outDir: supportDryRunDir,
          commit: false,
        }),
        commit: supportSaveDraftArgs({
          rowsFile: supportRowsFile,
          outDir: supportCommitDir,
          commit: true,
        }),
        post_write_verify: supportVerifyArgs({
          rowsFile: supportRowsFile,
          outDir: supportVerifyDir,
          targetUserId,
          stateCode,
        }),
      }
  ...
  const flowFinalizeArgs = flowFinalizeArgsSpec
    ? finalizeArgsWithDefaults(flowFinalizeArgsSpec.args, {
        outDir: path.join(outDir, "flow", "finalize"),
        targetUserId,
        stateCode,
      })
    : null;
  const processFinalizeArgs = processFinalizeArgsSpec
    ? finalizeArgsWithDefaults(processFinalizeArgsSpec.args, {
        outDir: path.join(outDir, "process", "finalize"),
        targetUserId,
        stateCode,
      })
    : null;
  let flowHandoff = flowFinalizeArtifact
    ? importCommitHandoffScope({
        datasetType: "flow",
        finalizeArtifact: flowFinalizeArtifact,
        targetUserId,
        stateCode,
        outDir: path.join(outDir, "flow", "handoff"),
        blockers,
      })
    : null;
  let processHandoff = processFinalizeArtifact
    ? importCommitHandoffScope({
        datasetType: "process",
        finalizeArtifact: processFinalizeArtifact,
        targetUserId,
        stateCode,
  ...
  if (supportCommands) {
    plannedStages.push(
      plannedCommandStage(
        "support_save_draft_dry_run",
        supportCommands.dry_run,
        knownSupportSaveDraftReport(supportDryRunDir),
      ),
      plannedCommandStage(
        "support_save_draft_commit",
        supportCommands.commit,
        knownSupportSaveDraftReport(supportCommitDir),
      ),
      plannedCommandStage(
        "support_post_write_verify",
        supportCommands.post_write_verify,
        knownSupportVerifyReport(supportVerifyDir),
      ),
    );
  }
  if (flowFinalizeArgs) {
    plannedStages.push(
      plannedCommandStage(
        "flow_post_authoring_finalize",
        foundryStageCommandArgs(flowFinalizeArgs),
        finalizeReportFileFromArgs(flowFinalizeArgs),
      ),
    );
  }
  if (flowHandoff) {
    plannedStages.push({
      stage: "flow_handoff_plan",
      status: flowHandoff.status,
      exit_code: flowHandoff.status === "ready_for_explicit_commit" ? 0 : 1,
      command_line: null,
      report_file: resolveRepoPath(flowHandoff.files?.report),
    });
  } else if (flowFinalizeArgs) {
    plannedStages.push({
      stage: "flow_handoff_plan",
      status: "planned_after_flow_finalize",
      exit_code: 0,
      command_line: null,
      report_file: path.join(outDir, "flow", "handoff", "dataset-commit-handoff-plan.json"),
    });
  }
  if (processFinalizeArgs) {
    plannedStages.push(
      plannedCommandStage(
        "process_post_authoring_finalize",
        foundryStageCommandArgs(processFinalizeArgs),
        finalizeReportFileFromArgs(processFinalizeArgs),
      ),
    );
  }
  if (processHandoff) {
    plannedStages.push({
      stage: "process_handoff_plan",
      status: processHandoff.status,
      exit_code:
        processHandoff.status === "ready_for_explicit_commit" ? 0 : 1,
      command_line: null,
      report_file: resolveRepoPath(processHandoff.files?.report),
    });
  } else if (processFinalizeArgs) {
    plannedStages.push({
      stage: "process_handoff_plan",
      status: "planned_after_process_finalize",
      exit_code: 0,
      command_line: null,
      report_file: path.join(
        ...
        "process",
        "handoff",
        "dataset-commit-handoff-plan.json",
      ),
    });
  }

  const reuseReadyFinalize = ({ datasetType, finalizeArgs }) => {
    if (!reuseReadyFinalizeReports || !finalizeArgs || blockers.length > 0) {
      return null;
    }
    const reusable = reusableFinalizeArtifactFromArgs({
      datasetType,
      finalizeArgs,
    });
    if (!reusable) return null;
    if (ensureArray(reusable.reuse_blockers).length > 0) {
      blockers.push(...reusable.reuse_blockers);
      stages.push({
        stage: `${datasetType}_post_authoring_finalize`,
        status: "blocked_reuse_candidate",
        exit_code: 1,
        command: "foundry.dataset-post-authoring-finalize",
        args: [],
        command_line: foundryStageCommandArgs(finalizeArgs)
          .map(shellQuote)
          .join(" "),
        stderr: "",
        report: reusable.value,
        report_file: reusable.path,
      });
      return null;
    }
    if (reusable.value?.status !== "ready_for_remote_write") {
      return null;
    }
    const reuseHandoffBlockers = [];
    const handoff = importCommitHandoffScope({
      datasetType,
      finalizeArtifact: reusable,
      targetUserId,
      stateCode,
      outDir: path.join(outDir, datasetType, "handoff"),
      blockers: reuseHandoffBlockers,
    });
    if (handoff?.status !== "ready_for_explicit_commit") return null;
    stages.push({
      stage: `${datasetType}_post_authoring_finalize`,
      status: "reused_ready_finalize_report",
      exit_code: 0,
      command: "foundry.dataset-post-authoring-finalize",
      args: [],
      command_line: foundryStageCommandArgs(finalizeArgs)
    ...
    stages.push({
      ...
      exit_code: 0,
      command: "foundry.dataset-commit-handoff-plan",
      args: [],
      command_line: null,
      stderr: "",
      report: handoff,
      report_file: resolveRepoPath(handoff.files?.report),
    });
    return handoff;
  };

  if (!commitRequested || blockers.length > 0) {
    if (reuseReadyFinalizeReports && blockers.length === 0) {
      for (const plannedStage of plannedStages) {
        if (plannedStage.stage.startsWith("support_")) {
          stages.push(plannedStage);
        }
      }
      if (!flowHandoff && flowFinalizeArgs) {
        flowHandoff = reuseReadyFinalize({
          datasetType: "flow",
          finalizeArgs: flowFinalizeArgs,
        });
      }
      if (!processHandoff && processFinalizeArgs) {
        processHandoff = reuseReadyFinalize({
          datasetType: "process",
          finalizeArgs: processFinalizeArgs,
        });
      }
    }
    const existingPlannedStages = new Set(stages.map((stage) => stage.stage));
    for (const plannedStage of plannedStages) {
      if (!existingPlannedStages.has(plannedStage.stage)) {
        stages.push(plannedStage);
      }
    }
  } else {
    const runDelayedFinalize = ({ datasetType, finalizeArgs }) => {
      if (!finalizeArgs || blockers.length > 0) return null;
      const finalizeStage = runJsonCommandStage(
        `${datasetType}_post_authoring_finalize`,
        foundryStageCommandArgs(finalizeArgs),
        finalizeReportFileFromArgs(finalizeArgs),
      );
      stages.push(finalizeStage);
      const finalizeBlocker = stageFailureBlocker(finalizeStage);
      if (finalizeBlocker) {
        blockers.push(finalizeBlocker);
        return null;
      }
      const finalizeReportPath = resolveRepoPath(
        finalizeStage.report?.files?.report ||
          finalizeStage.report_file ||
          finalizeReportFileFromArgs(finalizeArgs),
      );
      if (!finalizeReportPath || !fileExists(finalizeReportPath)) {
        blockers.push({
          code: `${datasetType}_finalize_report_missing`,
          stage: `${datasetType}_post_authoring_finalize`,
          message:
            "Delayed post-authoring finalize completed but no readable finalize report was found.",
          expected_report: repoRelativeMaybe(finalizeReportPath),
        });
        return null;
      }
      const handoff = importCommitHandoffScope({
        datasetType,
        finalizeArtifact: {
          path: finalizeReportPath,
          value: readJson(finalizeReportPath),
        },
        targetUserId,
        stateCode,
        outDir: path.join(outDir, datasetType, "handoff"),
      ...
      stages.push({
        ...
        exit_code:
          handoff?.status === "ready_for_explicit_commit" ? 0 : 1,
        command: "foundry.dataset-commit-handoff-plan",
        args: [],
        command_line: null,
        stderr: "",
    ...
    const runCommittedScope = ({ datasetType, handoff }) => {
      if (blockers.length > 0 || !handoff) return;
      const handoffDir = resolveRepoPath(path.dirname(handoff.files.report));
      const finalRowsFile = resolveRepoPath(handoff.final_rows_file);
      const commitArgs = commitCommandForDatasetType(
        datasetType,
        finalRowsFile,
        handoffDir,
        { targetUserId },
      ...
      const verifyArgs = [
        ...
        finalRowsFile,
        "--out-dir",
        path.join(handoffDir, "post-write-verify"),
        "--root-policy",
        "candidate",
        "--compare-root-payload",
        "--json",
      ...
      const commitStage = runJsonCommandStage(
        `${datasetType}_commit`,
        commitArgs,
        knownCommitReportForDatasetType(datasetType, handoffDir),
      );
      stages.push(commitStage);
      const commitBlocker = stageFailureBlocker(commitStage);
      if (commitBlocker) {
        blockers.push(commitBlocker);
        return;
      }
      const verifyStage = runJsonCommandStage(
        `${datasetType}_post_write_verify`,
        verifyArgs,
        knownPostWriteVerifyReport(handoffDir),
      );
      stages.push(verifyStage);
      const verifyBlocker = stageFailureBlocker(verifyStage);
      if (verifyBlocker) {
        blockers.push(verifyBlocker);
        return;
      }
      const closeout = runDatasetPostWriteCloseout({
        handoffPlan: resolveRepoPath(handoff.files.report),
        commitReport: knownCommitReportForDatasetType(datasetType, handoffDir),
        postWriteVerifyReport: knownPostWriteVerifyReport(handoffDir),
        outDir: path.join(handoffDir, "post-write-closeout"),
      });
      stages.push({
        stage: `${datasetType}_post_write_closeout`,
        status: closeout.status,
        exit_code: closeout.status === "completed" ? 0 : 1,
        command: "foundry.dataset-post-write-closeout",
        args: [],
        command_line: null,
        stderr: "",
        report: closeout,
        report_file: resolveRepoPath(closeout.files?.report),
      });
      if (closeout.status === "completed" && closeout.files?.report) {
        closeoutReportPaths.push(resolveRepoPath(closeout.files.report));
      }
      blockers.push(
        ...closeoutFailureBlockers(
          closeout,
          `${datasetType}_post_write_closeout`,
        ),
      );
    };

    if (supportCommands) {
      for (const [stage, args, reportFile] of [
        [
          "support_save_draft_dry_run",
          supportCommands.dry_run,
          knownSupportSaveDraftReport(supportDryRunDir),
        ...
        const result = runJsonCommandStage(stage, args, reportFile);
        stages.push(result);
        const blocker = stageFailureBlocker(result);
        if (blocker) {
    ...
    runCommittedScope({ datasetType: "flow", handoff: flowHandoff });
    if (!processHandoff && processFinalizeArgs) {
      processHandoff =
        reuseReadyFinalize({
          datasetType: "process",
    ...
    runCommittedScope({ datasetType: "process", handoff: processHandoff });
    if (blockers.length === 0 && closeoutReportPaths.length > 0) {
      importCompletionReport = runDatasetImportCompletionReport({
        closeoutReport: closeoutReportPaths,
        outDir: path.join(outDir, "import-completion"),
      });
      stages.push({
        stage: "dataset_import_completion_report",
        status: importCompletionReport.status,
        exit_code: importCompletionReport.status === "completed" ? 0 : 1,
        command: "foundry.dataset-import-completion-report",
        args: [],
        command_line: null,
        stderr: "",
        report: importCompletionReport,
        report_file: resolveRepoPath(importCompletionReport.files?.report),
      });
      if (importCompletionReport.status !== "completed") {
        blockers.push({
          code: "dataset_import_completion_report_not_completed",
          stage: "dataset_import_completion_report",
          message:
            "Ordered import write completed closeouts but task-level completion report is not completed.",
          completion_report: importCompletionReport.files?.report ?? null,
          completion_blockers: importCompletionReport.blockers ?? [],
        });
      }
    }
  }
  ...
  const completed =
    ...
    confirmRemoteWrite &&
    blockers.length === 0 &&
    stages.some((stage) => stage.status !== "planned");
  const reportPath = path.join(outDir, "dataset-import-commit-run-report.json");
  const report = {
    schema_version: 1,
    generated_at_utc: nowIso(),
    status:
    ...
    command: "dataset-import-commit-run",
    remote_write_mode: commitRequested ? "explicit-commit" : "read-only",
    target_user_id: targetUserId || null,
    expected_state_code: stateCode || null,
    reuse_ready_finalize_reports: reuseReadyFinalizeReports,
    policy: {
      ordered_write:
        "Ready support rows are written first, flow finalize closeout second, and process finalize closeout last, so process reference closure can rely on committed flow/support rows.",
      explicit_commit_required:
        "Without --commit this command only writes a read-only execution plan. Remote writes require --commit --confirm-remote-write.",
      sdk_validation_before_write:
        "Every commit stage uses the same tiangong-lca CLI commands as the individual handoff plans, preserving SDK validation before remote writes.",
      ready_finalize_reuse:
        "When --reuse-ready-finalize-reports is provided with delayed finalize args, Foundry may reuse an existing ready finalize report only after rows-file, mutation manifest, full-context proof, and commit handoff gates pass.",
    },
    inputs: {
      support_rows: repoRelativeMaybe(supportRowsFile),
      flow_finalize_report: flowFinalizeArtifact
        ? repoRelativePath(flowFinalizeArtifact.path)
        : null,
      flow_finalize_args_file: flowFinalizeArgsSpec
        ? repoRelativePath(flowFinalizeArgsSpec.path)
        : null,
      process_finalize_report: processFinalizeArtifact
        ? repoRelativePath(processFinalizeArtifact.path)
        : null,
      process_finalize_args_file: processFinalizeArgsSpec
        ? repoRelativePath(processFinalizeArgsSpec.path)
        : null,
    },
    counts: {
      blockers: blockers.length,
      planned_stages: stages.length,
      support_rows:
        supportRowsFile && fileExists(supportRowsFile)
          ? countRowsFile(supportRowsFile)
          : 0,
      flow_write_candidates:
        flowHandoff?.counts?.write_candidates ??
        flowFinalizeArtifact?.value?.counts?.write_candidates ??
        0,
      process_write_candidates:
        processHandoff?.counts?.write_candidates ??
        processFinalizeArtifact?.value?.counts?.write_candidates ??
        0,
      completed_stages: commitRequested
        ? stages.filter(
            (stage) =>
              stage.status !== "planned" &&
              stage.exit_code === 0 &&
              !String(stage.status).includes("blocked"),
          ).length
        : 0,
      closeout_reports: closeoutReportPaths.length,
      import_completion_status: importCompletionReport?.status ?? null,
    },
    blockers,
    stages: stages.map((stage) => ({
      stage: stage.stage,
      status: stage.status ?? stage.report?.status ?? null,
      exit_code: stage.exit_code,
      command: stage.command ?? null,
      args: stage.args ?? [],
      command_line: stage.command_line ?? null,
      report_file: repoRelativeMaybe(stage.report_file),
      parse_error: stage.parse_error ?? null,
    })),
    commands: {
      support: supportCommands
        ? {
            dry_run: supportCommands.dry_run.map(shellQuote).join(" "),
            commit: supportCommands.commit.map(shellQuote).join(" "),
            post_write_verify: supportCommands.post_write_verify
              .map(shellQuote)
              .join(" "),
          }
        : null,
      flow_finalize: flowFinalizeArgs
        ? foundryStageCommandArgs(flowFinalizeArgs).map(shellQuote).join(" ")
        : null,
      flow: flowHandoff?.commands ?? null,
      process_finalize: processFinalizeArgs
        ? foundryStageCommandArgs(processFinalizeArgs).map(shellQuote).join(" ")
        : null,
      process: processHandoff?.commands ?? null,
    },
    files: {
      report: repoRelativePath(reportPath),
      flow_finalize_report: flowFinalizeArgs
        ? repoRelativeMaybe(finalizeReportFileFromArgs(flowFinalizeArgs))
        : null,
      flow_handoff_plan:
        flowHandoff?.files?.report ??
        (flowFinalizeArgs
          ? repoRelativePath(
              path.join(
                outDir,
          ...
          : null),
      process_finalize_report: processFinalizeArgs
        ? repoRelativeMaybe(finalizeReportFileFromArgs(processFinalizeArgs))
        : null,
      process_handoff_plan:
        processHandoff?.files?.report ??
        (processFinalizeArgs
          ? repoRelativePath(
              path.join(
                outDir,
          ...
          : null),
      support_commit_report: supportRowsFile
        ? repoRelativePath(knownSupportSaveDraftReport(supportCommitDir))
        : null,
      support_post_write_verify_report: supportRowsFile
        ? repoRelativePath(knownSupportVerifyReport(supportVerifyDir))
        : null,
      flow_closeout_report: flowHandoff
        ? repoRelativePath(
            path.join(
              path.dirname(resolveRepoPath(flowHandoff.files.report)),
              "post-write-closeout",
              "dataset-post-write-closeout-report.json",
            ),
          )
        : null,
      process_closeout_report: processHandoff
        ? repoRelativePath(
            path.join(
              path.dirname(resolveRepoPath(processHandoff.files.report)),
              "post-write-closeout",
              "dataset-post-write-closeout-report.json",
            ),
          )
        : null,
      import_completion_report: repoRelativePath(
        path.join(
          outDir,
          "import-completion",
...
function deriveSupabaseFunctionsBaseUrl(apiBaseUrl) {
...
function countFromContentRange(value) {
  const match = asText(value).match(/\/(\d+)$/u);
  return match ? Number(match[1]) : 0;
}

async function countStateCodeRows({
  projectUrl,
  publishableKey,
  accessToken,
  table,
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(
      `Count failed for ${table}: ${response.status} ${response.statusText} ${text}`,
    );
  }
  return countFromContentRange(response.headers.get("content-range"));
}

async function deleteStateCodeRows({
  projectUrl,
  publishableKey,
  accessToken,
  table,
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(
      `Delete failed for ${table}: ${response.status} ${response.statusText} ${text}`,
    );
  }
}

async function listStateCodeRows({
  projectUrl,
  publishableKey,
  accessToken,
  table,
...
async function deleteDatasetRowViaEdge({
  functionsUrl,
  publishableKey,
  accessToken,
  table,
  ...
  const { payload } = await supabaseJsonRequest(
    `${functionsUrl}/app_dataset_delete`,
    {
      method: "POST",
      headers: {
        apikey: publishableKey,
      ...
      body: JSON.stringify({ table, id, version }),
    },
  );
  if (payload?.ok !== true) {
    throw new Error(
      `app_dataset_delete failed for ${table}:${id}@${version}: ${JSON.stringify(payload)}`,
    );
  }
}

async function runDatasetTestAccountClearState0(options) {
  if (options.help) {
    return {
      schema_version: 1,
      status: "help",
      command: "dataset-test-account-clear-state0",
      usage: [
        "node scripts/foundry.mjs dataset-test-account-clear-state0 --confirm-account <test-account-email> --confirm-delete-state-code-0",
      ],
      purpose:
        "Delete all state_code=0 rows owned by the configured test account from dataset tables before a real import smoke test.",
      remote_write_mode: "destructive-delete",
    };
  }
  ...
  const functionsUrl = deriveSupabaseFunctionsBaseUrl(
    process.env.TIANGONG_LCA_API_BASE_URL,
  );
  const publishableKey = asText(
    process.env.TIANGONG_LCA_SUPABASE_PUBLISHABLE_KEY,
  ...
  const credentials = decodeUserApiKey(process.env.TIANGONG_LCA_API_KEY);
  const confirmAccount = asText(
    options.confirmAccount || options.account || options.email,
  );
  const confirmDelete = booleanOption(
    options.confirmDeleteStateCode0 ||
      options.confirmDeleteStateCodeZero ||
      options["confirmDeleteStateCode-0"],
  );
  if (!publishableKey) {
    throw new Error("TIANGONG_LCA_SUPABASE_PUBLISHABLE_KEY is required.");
  }
  if (confirmAccount !== credentials.email) {
    throw new Error(
      `Refusing to clear test account. --confirm-account must equal configured account ${credentials.email}.`,
    );
  }
  if (!confirmDelete) {
    throw new Error(
      "Refusing to clear data without --confirm-delete-state-code-0.",
    );
  }
  ...
  const tables = normalizedList(options.table || options.tables);
  const tableOrder =
    tables.length > 0
      ? tables
      : [
  ...
  for (const table of tableOrder) {
    const before = await countStateCodeRows({
      projectUrl,
      publishableKey,
      accessToken: session.accessToken,
      table,
    ...
    const candidates =
      before > 0
        ...
        : [];
    let deleteMethod = candidates.length > 0 ? "edge_function" : "none";
    const deleteErrors = [];
    if (candidates.length > 0) {
      try {
        for (const candidate of candidates) {
          await deleteDatasetRowViaEdge({
            functionsUrl,
            publishableKey,
            accessToken: session.accessToken,
            table,
        ...
        deleteMethod = "rest_bulk_fallback";
      }
    }
    const after = await countStateCodeRows({
      projectUrl,
      publishableKey,
      accessToken: session.accessToken,
      table,
    ...
    rows.push({
      table,
      before,
      listed_candidates: candidates.length,
      deleted: Math.max(0, before - after),
      after,
      delete_method: deleteMethod,
      delete_errors: deleteErrors,
      status: after === 0 ? "cleared" : "not_empty",
    });
  }
  ...
  return {
    schema_version: 1,
    generated_at_utc: nowIso(),
    status: rows.every((row) => row.after === 0)
      ? "completed"
    ...
    counts: {
      ...
      deleted: rows.reduce((total, row) => total + row.deleted, 0),
  ...
  if (options.help) {
    return {
      ...
      usage: [
        ...
        "npm run legacy:dataset:identity-preflight:run -- --index ./identity-preflight-requests/identity-preflight-requests.jsonl --only-pending --timeout-ms 60000",
      ],
      remote_write_mode: "read-only",
    };
  }
  ...
  if (options.help) {
    return {
      ...
      usage: [
        ...
        "npm run legacy:dataset:identity-preflight-index:merge -- --base-index ./identity-preflight-requests.jsonl --update-index ./fresh/identity-preflight-requests.jsonl",
      ],
      remote_write_mode: "read-only",
    };
  }
...
function usage() {
  ...
  const legacyDatasetCommands = [
    ...
    "dataset-identity-decisions-apply",
    "dataset-test-account-clear-state0",
    "dataset-post-authoring-finalize",
    "dataset-commit-handoff-plan",
    "dataset-import-commit-run",
  ...
  return {
    ...
    legacy_dataset_commands: legacyDatasetCommands,
    commands: [...publicCommands, ...legacyDatasetCommands],
    migration_note:
      "Foundry public surface is task/profile/workspace/gate control. Legacy dataset commands remain for migration only; durable conversion, queue state, validation, QA, write, and verify behavior should live in tiangong-lca CLI or checked-in skills.",
  };
}
...
async function main() {
  ...
  switch (command) {
    ...
    case "dataset-identity-decisions-apply":
      ...
      exitCode = ["help", "completed"].includes(result.status) ? 0 : 1;
      break;
    case "dataset-test-account-clear-state0":
      result = await runDatasetTestAccountClearState0(options);
      exitCode = ["help", "completed"].includes(result.status) ? 0 : 1;
      break;
    ...
    case "dataset-commit-handoff-plan":
      ...
      exitCode = ["help", "ready_for_explicit_commit"].includes(result.status)
        ...
        : 1;
      break;
    case "dataset-import-commit-run":
      result = runDatasetImportCommitRun(options);
      exitCode = [
        "help",
        "ready_for_explicit_commit",
        "completed",
