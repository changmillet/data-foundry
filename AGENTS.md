...
- Keep user-facing clarification in the Skills-owned entry. Foundry registers the optional task brief and exact question/answer/assumption history, projects only applicable current decisions to authoring, and records which decisions semantic work used. Present a human question as soon as its bounded assessment finds a real decision gap; incomplete assessment cannot finalize. Natural-language answers and AI assumptions are task context, not scientific facts, profile defaults, waiver or write approval. A decision recap is derived from current evidence and adds no completion confirmation gate.
