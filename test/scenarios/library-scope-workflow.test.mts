...
test("published CLI resolver binds the installed 0.1.19 package bin through Node", () => {
  ...
  try {
    ...
    assert.equal(cli.package, "@tiangong-lca/cli@0.1.19");
    assert.equal(cli.package_version, "0.1.19");
...
test("installed CLI schemas resolve independently of cwd and sibling worktrees", () => {
  ...
  assert.equal(version.stdout.trim(), "0.1.19");
});
