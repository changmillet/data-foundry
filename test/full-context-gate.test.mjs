...
test("dataset import commit run plans support then flow then process", () => {
  const root = path.join(fixtureRoot, "ordered-import-commit-run-plan");
  fs.rmSync(root, { recursive: true, force: true });
  const supportRows = path.join(root, "support.jsonl");
  const flowRows = path.join(root, "flows.jsonl");
  const processRows = path.join(root, "processes.jsonl");
  writeJsonLines(supportRows, [{ contactDataSet: { contactInformation: {} } }]);
  writeJsonLines(flowRows, [{ id: "flow-a" }]);
  writeJsonLines(processRows, [{ id: "process-a" }]);
  const flow = writeReadyFinalizeFixture({
    root,
    datasetType: "flow",
    rowsFile: flowRows,
  });
  const process = writeReadyFinalizeFixture({
    root,
    datasetType: "process",
    rowsFile: processRows,
  });

  try {
    const planned = runFoundry([
      "dataset-import-commit-run",
      "--support-rows",
      rel(supportRows),
      "--flow-finalize-report",
      rel(flow.finalizeReport),
      "--process-finalize-report",
      rel(process.finalizeReport),
      "--target-user-id",
      targetUserId,
      "--state-code",
      "0",
      "--out-dir",
      rel(path.join(root, "commit-run")),
    ]);
    assert.equal(planned.code, 0, JSON.stringify(planned.json, null, 2));
    assert.equal(planned.json.status, "ready_for_explicit_commit");
    assert.deepEqual(
      planned.json.stages.map((stage) => stage.stage),
      [
    ...
    assert.match(
      planned.json.commands.support.commit,
      / dataset save-draft /,
    );
    assert.match(planned.json.commands.flow.commit, / flow publish-version /);
    assert.match(
      planned.json.commands.flow.commit,
      new RegExp(`--target-user-id ${targetUserId}`),
    );
    assert.match(planned.json.commands.process.commit, / process save-draft /);
    assert.doesNotMatch(
      planned.json.commands.process.commit,
      /--target-user-id/,
    );
    assert.match(
      planned.json.commands.process.post_write_verify,
      /--state-code 0/,
    );
    assert.ok(
      fs.existsSync(path.join(root, "commit-run", "flow", "handoff")),
    );
    assert.ok(
      fs.existsSync(path.join(root, "commit-run", "process", "handoff")),
    );
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
...
test("dataset import commit run requires explicit remote write confirmation", () => {
  const root = path.join(fixtureRoot, "ordered-import-commit-run-confirmation");
  fs.rmSync(root, { recursive: true, force: true });
  const flowRows = path.join(root, "flows.jsonl");
  writeJsonLines(flowRows, [{ id: "flow-a" }]);
  const flow = writeReadyFinalizeFixture({
  ...
  try {
    ...
    assert.equal(blocked.code, 1);
    assert.equal(blocked.json.status, "blocked");
    assert.ok(blockerCodes(blocked.json).has("confirm_remote_write_required"));
    assert.equal(
      blocked.json.stages.every((stage) => stage.status !== "completed"),
      true,
    );
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
...
test("dataset import commit run can plan delayed flow and process finalize", () => {
  const root = path.join(fixtureRoot, "ordered-import-delayed-finalize-plan");
  fs.rmSync(root, { recursive: true, force: true });
  const supportRows = path.join(root, "support.jsonl");
  const flowRows = path.join(root, "flows.jsonl");
  const processRows = path.join(root, "processes.jsonl");
  const flowFinalizeArgsFile = path.join(root, "flow-finalize-args.json");
  const processFinalizeArgsFile = path.join(root, "process-finalize-args.json");
  writeJsonLines(supportRows, [{ contactDataSet: { contactInformation: {} } }]);
  writeJsonLines(flowRows, [{ id: "flow-a" }]);
  writeJsonLines(processRows, [{ id: "process-a" }]);
  writeJson(flowFinalizeArgsFile, [
  ...
  writeJson(processFinalizeArgsFile, {
    args: [
      "dataset-post-authoring-finalize",
      "--type",
      "process",
  ...
  try {
    const planned = runFoundry([
      ...
      "--support-rows",
      rel(supportRows),
      "--flow-finalize-args-file",
      rel(flowFinalizeArgsFile),
      "--process-finalize-args-file",
      rel(processFinalizeArgsFile),
      "--target-user-id",
      targetUserId,
      "--state-code",
      "0",
    ...
    assert.equal(
      planned.json.stages.find((stage) => stage.stage === "flow_handoff_plan")
        .status,
      "planned_after_flow_finalize",
    );
    assert.equal(
      planned.json.stages.find(
        (stage) => stage.stage === "process_handoff_plan",
      ).status,
      "planned_after_process_finalize",
    );
    assert.match(
      planned.json.commands.flow_finalize,
      / dataset-post-authoring-finalize /,
    );
    assert.match(
      planned.json.commands.flow_finalize,
    ...
    assert.match(planned.json.commands.flow_finalize, /--state-code 0/);
    assert.equal(planned.json.commands.flow, null);
    assert.equal(planned.json.commands.process, null);
    assert.match(
      planned.json.files.flow_finalize_report,
      /flow\/finalize\/dataset-post-authoring-finalize-report\.json$/,
    );
    assert.match(
      planned.json.files.process_finalize_report,
      /process\/finalize\/dataset-post-authoring-finalize-report\.json$/,
    );
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
...
test("dataset import commit run can reuse ready delayed finalize reports", () => {
  const root = path.join(fixtureRoot, "ordered-import-delayed-finalize-reuse");
  fs.rmSync(root, { recursive: true, force: true });
  const supportRows = path.join(root, "support.jsonl");
  const flowRows = path.join(root, "flows.jsonl");
  const processRows = path.join(root, "processes.jsonl");
  const outDir = path.join(root, "commit-run");
  const flowFinalizeArgsFile = path.join(root, "flow-finalize-args.json");
  const processFinalizeArgsFile = path.join(root, "process-finalize-args.json");
  writeJsonLines(supportRows, [{ contactDataSet: { contactInformation: {} } }]);
  writeJsonLines(flowRows, [{ id: "flow-a" }]);
  ...
  writeReadyFinalizeFixture({
    root: path.join(root, "flow-ready"),
    datasetType: "flow",
    rowsFile: flowRows,
    finalizeReportPath: path.join(
      outDir,
      "flow",
      "finalize",
  ...
  writeReadyFinalizeFixture({
    root: path.join(root, "process-ready"),
    datasetType: "process",
    rowsFile: processRows,
    finalizeReportPath: path.join(
      outDir,
  ...
  try {
    ...
    assert.equal(planned.code, 0, JSON.stringify(planned.json, null, 2));
    assert.equal(planned.json.status, "ready_for_explicit_commit");
    assert.equal(planned.json.reuse_ready_finalize_reports, true);
    assert.deepEqual(
      planned.json.stages.map((stage) => [stage.stage, stage.status]),
      [
        ["support_save_draft_dry_run", "planned"],
        ["support_save_draft_commit", "planned"],
        ["support_post_write_verify", "planned"],
        ["flow_post_authoring_finalize", "reused_ready_finalize_report"],
        ["flow_handoff_plan", "ready_for_explicit_commit"],
        ["process_post_authoring_finalize", "reused_ready_finalize_report"],
        ["process_handoff_plan", "ready_for_explicit_commit"],
      ],
    );
    assert.match(planned.json.commands.flow.commit, / flow publish-version /);
    assert.match(planned.json.commands.process.commit, / process save-draft /);
    assert.notEqual(planned.json.commands.flow_finalize, null);
    assert.notEqual(planned.json.commands.process_finalize, null);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
