...
lastReviewedNote: "Reviewed for Foundry #200: queue exit-1 blockers stay in local task evidence while independent assessment proceeds; stage order, CLI ownership, authorization and no-replay boundaries remain unchanged."
tracker:
  kind: filesystem
  inbox: tasks/inbox
  active: tasks/active
...
For consumer workspace execution, follow `docs/public-runtime-contract.md`. Use `workspace init`, `doctor`, `task start`, `task status`, `task resume` and `workspace migrate --dry-run` with `--json`. Task specs own request/actor/lane/profile/source/seed/account/preparation intent and an optional concise brief; status and resume require the same explicit actor. When a current blocker needs user judgment, the Skills entry explains the missing facts, effect and recommended next step plainly, registers the question and raw answer through `--interaction-input`, and continues independent work. The current answer and applicable AI assumptions enter authoring context; incomplete assessment coverage cannot finalize. A completed task provides a derived decision recap without another approval step. Start and local preparation require no login. Execute only returned structured argv actions with their recorded CWD/binding; never run display text. A nonempty attempt state is readback-only, and W10 alone may apply a migration plan.
...
Use the exact installed project dependency as `pnpm exec tiangong-lca ...`. Foundry runtime adapters resolve that same `@tiangong-lca/cli@0.1.19` manifest and bin directly; only the external `skills@latest` source-evidence resolver remains intentionally floating, with the resolved ref recorded in task artifacts.
