...
For consumer workspace execution, follow `docs/public-runtime-contract.md`. Use `workspace init`, `doctor`, `task start`, `task status`, `task resume` and `workspace migrate --dry-run` with `--json`. Task specs own request/actor/lane/profile/source/seed/account/preparation intent and an optional concise brief; status and resume require the same explicit actor. When a current blocker needs user judgment, the Skills entry explains the missing facts, effect and recommended next step plainly, registers the question and raw answer through `--interaction-input`, and continues independent work. A choice for one object must carry its exact current registered row hash, identity and version; task/type-wide choices remain explicit. Status identifies the affected object and exposes separate current authoring context for independent objects. Initial answers still permit local assessment; every narrow answer needs indexed adoption before authorization or execution, and an unproven changed row cannot inherit an old choice. Authorization and owner execution recheck current interaction evidence; after indexed authorization, changed choices require a separately reviewed task revision. Incomplete assessment coverage cannot finalize. A completed task provides a derived decision recap without another approval step. Start and local preparation require no login. Execute only returned structured argv actions with their recorded CWD/binding; never run display text. A nonempty attempt state is readback-only, and W10 alone may apply a migration plan.
